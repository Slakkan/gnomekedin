...
export function paginate<T>(array: T[], itemsPerPage: number, currentPageIndex: number): T[] {
  ...
  if (array.length <= itemsPerPage) {
    ...
    return isPageComplete ? array.splice(itemsPerPage * currentPageIndex, itemsPerPage) :
      array.splice(itemsPerPage * currentPageIndex, array.length - (itemsPerPage * currentPageIndex));
